...
def generate_schedule(nurses, year, month, history, public_holidays):
    ...
    shifts = {}
    phase_nuit = {} # Phase par semaine pour garantir les blocs long terme
    ...
    for n in nurses:
        for d in range(1, num_days + 1):
            week = (d - 1) // 7
            # Gestion des phases (TIIH et JOUR uniquement en phase JOUR)
            ...
            if d < num_days:
                model.Add(shifts[(n, d, 1)] + shifts[(n, d+1, 0)] <= 1) # Pas Nuit -> Jour
    ...
    if solver.Solve(model) in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
        return {n: {d: ("☀️ JOUR" if solver.Value(shifts[(n, d, 0)]) else "🌙 NUIT" if solver.Value(shifts[(n, d, 1)]) else "🛠️ TIIH" if solver.Value(shifts[(n, d, 2)]) else "-") for d in range(1, num_days + 1)} for n in nurses}
    return None
